...
router.post('/generate-report', verifyAuth, async (req, res) => {
    try {
        ...
        // Fetch contract
        const { data: contract } = await req.supabase
            .from('contracts')
            .select('*')
            .eq('id', contractId)
            .eq('user_id', userId)
        ...
        doc
            .fontSize(24)
            .fillColor('#2563EB')
            .text('ContractAI', { align: 'center' })
            .fontSize(10)
            .fillColor('#666')
            .text('AI-Powered Contract Analysis Report', { align: 'center' })
            .moveDown()
        ...
        if (clauses.length > 0) {
            ...
            clauses.forEach((clause, i) => {
                doc.fontSize(12).fillColor('#EF4444')
                    .text(`${i + 1}. ${clause.title || 'Risky Clause'}`)
                doc.fontSize(10).fillColor('#4B5563')
                    .text(clause.description || clause.text || '', { lineGap: 3 })
                if (clause.severity) {
        ...
        if (alts.length > 0) {
            ...
            alts.forEach((alt, i) => {
                doc.fontSize(12).fillColor('#2563EB')
                    .text(`${i + 1}. ${alt.title || 'Alternative'}`)
                if (alt.original) {
        ...
        doc.fontSize(8).fillColor('#9CA3AF')
            .text('Generated by ContractAI • This report is for informational purposes only and does not constitute legal advice.', 50, doc.page.height - 40, { align: 'center' })
